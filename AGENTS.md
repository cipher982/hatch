...
Headless runner with a simple Claude/Codex/z.ai surface plus raw escape hatches when needed. Zero dependencies, stdlib only.
...
```
cli.py / runner.py → credentials.py → backends.py → subprocess(opencode/claude/codex/gemini)
                        ↓
                   infisical-get.py
           ↓
